...
const EmployeeTable = () => {
    ...
    const handleNavigateToEdit = (e, employeeId) => {
        ...
        user?.role === "admin"
            ...
            : user?.role === "hr"
                ? navigate(`/hr-dashboard/edit-employee/${employeeId}`)
                : navigate("/");
    };
    ...
    const handleConfirmDelete = async () => {
        const { payload } = await dispatch(
            deleteEmployeeById({ privateAxios, id: employeeToDelete }),
        );
        if (payload?.success) {
            dispatch(getEmployees({ accessToken: user.accessToken, privateAxios }));
        }
                            ...
                            .map((empData) => (
                                <tr key={empData?._id}>
                                    ...
                                    <td>
                                        <div>
                                            {empData?.createdAt
                                                ? new Date(empData.createdAt).toDateString()
                                                : "not available"}
                                        </div>
                                    </td>
                                    {empData?.status && (
                                    ...
                                    <td>
                                        <button
                                            className="text-xl"
                                            onClick={() => handleOpenModal(empData?._id)}
                                        >
                                            <MdDelete />
                                        </button>
                                    </td>
